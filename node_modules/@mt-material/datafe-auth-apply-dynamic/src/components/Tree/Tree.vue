...
export default class Tree extends Vue {
  ...
  public handleScroll = throttle(() => {
    this.$refs.scroller && this.updateVisibleData((this.$refs.scroller as HTMLElement).scrollTop);
  }, 50);
  public scrollTop = 0;
  public observer: any = {};
  ...
  public onTreeChange(val: any[], oldVal: any[]) {
    if (!isEqual(val, oldVal)) {
      this.flatData = flatten(val, 1, null, {
        props: this._props_,
  ...
  public onFlatDataChange(val: ITreeNode[]) {
    this.$refs.scroller && this.updateVisibleData((this.$refs.scroller as HTMLElement).scrollTop, val); // 重新渲染可视化数据
  }
  @Watch('checkedKeys')
  ...
  public updateVisibleData(scrollTop = 0, flatData = this.flatData) {
    ...
    let start = Math.floor(scrollTop / this.option.itemHeight) - Math.floor(visibleCount / 2);
      ...
      for (let i = 0; i < flatData.length; i++) {
        ...
        if (i > index) {
          ...
          if (flatData[i].expand) {
            const parentNode = flatData.find(item => item[id] === flatData[i].pid);
  ...
  public dragStart(item: ITreeNode) {
    ...
    this.dragItem = item;
    // 遍历找出被拖拽节点的全部子孙节点
    this.dragItemDescendants = findDescendants(this.flatData, item, this._props_);
    // 所在位置
    this.dragIndex = this.flatData.findIndex(node => node[id] === item[id]);
    this.dragLevel = item.level; // 拖拽节点等级
    const dragVisibleIndex = this.visibleData.findIndex(node => node[id] === item[id]);
  ...
  public dragOver(item: ITreeNode, e: DragEvent) {
    ...
    if (this.dragItem[id] !== item[id]) {
      ...
      if (top + 0.25 * itemHeight <= pageY && pageY <= top + 0.75 * itemHeight) {
  ...
  public drop(item: ITreeNode) {
    ...
    if (
      ...
      // 仅有一个根节点，不允许往根节点拖拽
      (flatData.filter(item => item.level === 1).length === 1 && item.level === 1)
    ) {
      // 拖拽完毕清空接收项数据，消除标线
      this.dropClearData();
      return;
    ...
    if (this.dragItemDescendants.length) {
      ...
      const index = flatData.findIndex(node => node[id] === this.dragItem[id]);
      flatData.splice(index + 1, 0, ...this.dragItemDescendants); // 需要放到接收项的下面，所以 index + 1
    }
    flatData.forEach((node, i) => {
      // 完善拖拽项父节点和接收项父节点的数据
      if (node[id] === this.dragItem.pid || node[id] === item.pid) {
        const childLen = flatData.filter(item => item.pid === node[id]).length;
        this.$set(flatData, i, {
          ...flatData[i],
          childLen,
          expand: !!childLen
  ...
  public eldersNodeCheckedHandler(itemElders: string[]) {
    ...
    for (index = 0; index < itemElders.length; index++) {
      ...
      const nodeAllChecked = nodeChildren.every((node: ITreeNode) => node[checked] || node[disableCheckbox]);
      const nodeAllNotChecked = nodeChildren.every((node: ITreeNode) => !node[checked] && !node[indeterminate]);
      const isIndeterminate = nodeAllChecked || nodeAllNotChecked ? false : true;
    ...
    if (index < itemElders.length) {
      ...
      for (let i = index + 1; i < itemElders.length; i++) {
        // 找到父节点在 flatData 中的位置
        const parentIndex = flatData.findIndex(item => item[id] === itemElders[i]);
        this.$set(flatData, parentIndex, {
          ...flatData[parentIndex],
          [checked]: false,
          [indeterminate]: true
  ...
  public getCheckedKeys() {
    const { id, checked } = this._props_;
    return this.flatData.filter(node => node[checked]).map(item => item[id]);
  }
  // 获取树的已选择节点
  public getCheckedTreeNode() {
    const { checked } = this._props_;
    return this.flatData.filter(node => node[checked]);
  }
  // 获取树的已展开节点
  ...
  // 删除树节点
  public deleteTreeNode(id: string) {
    const { flatData } = this;
    const item = flatData.find(node => node[this._props_.id] === id);
    if (!item) {
      return;
    }
    const itemIndex = flatData.findIndex(node => node[this._props_.id] === id);
    const itemDescendants = []; // 获取删除项的子孙
    for (let i = 0; i < flatData.length; i++) {
      if (i < itemIndex) {
        if (item.pid === flatData[i][this._props_.id]) {
          // 找出父节点 childLen - 1
          this.$set(flatData, i, { ...flatData[i], childLen: flatData[i].childLen - 1 });
    ...
    // 返回删除的节点 id
    return [item[this._props_.id], ...itemDescendants.map(item => item[this._props_.id])];
  }
  // 更新树节点
