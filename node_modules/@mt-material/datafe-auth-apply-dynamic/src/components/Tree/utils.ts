...
const getParentStatus = (children: ITreeNode[], option: IOption) => {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (expandKeys.includes(item[id]) || item.expand || item.visible) expandLen++;
  }
...
export const flatten = (
  ...
  const arr: ITreeNode[] = [];
  const {
    props: { title, children, id, checkable, checked, indeterminate, disableCheckbox, disableCheckboxHoverText },
    extProps,
    defaultActiveKey,
    defaultExpandAll,
    expandKeys,
  ...
  data.forEach((item: any) => {
    ...
    let itemChecked = parentChecked || checkedKeys.includes(item[id]) ? true : checked in item ? item[checked] : false;
    const childLen = item[children] ? item[children].length : 0;
    // expandKeys 包含该节点，且该节点有子节点，节点展开
    let itemExpand = childLen && (defaultExpandAll || expandKeys.includes(item[id]));
    // 如果节点禁用，则不勾选，不展开
    ...
    const newItem: ITreeNode = {
      ...
      // 默认展示勾选框
      [indeterminate]: false,
      [disableCheckbox]: disableCheckbox in item ? item[disableCheckbox] : false,
      [disableCheckboxHoverText]: disableCheckboxHoverText in item ? item[disableCheckboxHoverText] : '',
      level,
      expand: itemExpand,
      // 父节点展开，则子节点展示
      // 自身节点展开，则自己展示
      visible: level === 1 || defaultExpandAll || expandKeys.includes(item[id]) || !!parentExpand,
      pid,
      childLen,
    ...
    if (item[children] && item[children].length) {
      const newItemChildren = flatten(item[children], level + 1, item[id], option, itemChecked, itemExpand);
 ...
 * @returns
 */
export const findDescendants = (data: ITreeNode[], item: ITreeNode, props: IProps) => {
  const { id } = props;
  const arr = [];
 ...
 * @returns
 */
export const findElders = (data: ITreeNode[], item: ITreeNode, itemIndex: number, props: IProps) => {
  const { id } = props;
...
// 需要删除的属性
const deleteAttrKey = ['pid', 'level', 'expand', 'visible', 'active', 'childLen', 'loading', 'hasAsyncChildren'];
