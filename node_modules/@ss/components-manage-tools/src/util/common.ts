        ...
        if (isNotFoundError(error.message)) {
            try {
                ...
                if (isNotFoundError(importGlobalErr.message)) {
                    ...
                    await sync('npm', ['config', 'set', 'puppeteer_download_host=http://npm.sankuai.com/mirrors']);
                    const result = sync(
                        'npm',
                        ['install', 'puppeteer@1.x', '-g', '--registry', 'https://registry.npm.taobao.org'],
                        { stdio: 'inherit' },
                    );
                    await sync('npm', ['config', 'delete', 'puppeteer_download_host']);
...
export async function findReactPort() {
