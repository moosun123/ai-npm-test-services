<template>
  <div :class="{
      [`${prefix}-wrapper`]: true,
      [`${prefix}-group`]: $slots.prepend || $slots.append,
  ...
  >
    <div :class="`${prefix}-group-prepend`" v-if="$slots.prepend">
      <slot name="prepend" />
    </div>
    ...
    <div :class="`${prefix}-group-append`" v-if="$slots.append">
      <slot name="append" />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    hasSuffix() {
      return this.loading || this.clearable ||
        !!(this.suffixIcon || this.$slots.suffix);
    },
    hasValue() {
